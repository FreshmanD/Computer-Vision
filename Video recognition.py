...
while(True):
    ...
    ##    gray = cv.cvtColor(res,cv.COLOR_HSV2BGR)
        ...
        thresh = cv.adaptiveThreshold(blur,127,1,cv.THRESH_BINARY_INV,11,2)
        edge = cv.Canny(thresh,70,70)

        cnt2,hierarchy1 = cv.findContours(edge,cv.RETR_EXTERNAL,cv.CHAIN_APPROX_SIMPLE)
        ...
        for i in range(len(cnts3)):
            ...
            if size[0]*size[1] > 0:
                ...
                img1 = cv.bitwise_and(roi,roi,mask = mask)
